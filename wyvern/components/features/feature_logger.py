...
class FeatureEventLoggingComponent(
    ...
    async def execute(
        ...
        def feature_event_generator():
            ...
            feature_columns = input.feature_df.df.columns[1:]
            ...
            return [
                FeatureEvent(
                    ...
                    event_data=FeatureLogEventData(
                        # todo (kerem): What about composite identifiers? We only get the primary identifier here.
                        feature_identifier_type=row[IDENTIFIER].split(":")[0],
                        feature_identifier=row[IDENTIFIER].split(":")[1],
                        feature_name=col,
                        feature_value=row[col],
                    ),
                )
                for row in input.feature_df.df.iter_rows(named=True)
